...
func (cfg *config) start1(i int) {
	...
	go func() {
		for m := range applyCh {
			err_msg := ""
			//fmt.Printf("Found cmd %s in applyCh from srv %d\n", m.Command, i)
			if v, ok := (m.Command).(int); ok {
				...
				//fmt.Printf("Writing cfg log for srv %d at %d\n", i, m.Index)
...
func (cfg *config) one(cmd int, expectedServers int) int {
	...
	for time.Since(t0).Seconds() < 10 {
		...
		for si := 0; si < cfg.n; si++ {
			...
			// fmt.Printf("One calling start on srv %d with command %s\n", starts, cmd)
			var rf *Raft
			cfg.mu.Lock()
			...
			if rf != nil {
				...
				if ok {
					...
					// fmt.Printf("Found leader %d, command %s submitted\n", starts, cmd)
					break
				}
			}
		}
		...
		if index != -1 {
			// fmt.Printf("Index returned by start is %d\n", index)
			// somebody claimed to be the leader and to have
			// submitted our command; wait a while for agreement.
			t1 := time.Now()
			for time.Since(t1).Seconds() < 2 {
				nd, cmd1 := cfg.nCommitted(index)
				// fmt.Printf("nCommitted result %d, %s\n", nd, cmd1)
